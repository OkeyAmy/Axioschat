...
export const getTokenBalance = async (web3: any, tokenAddress: string, walletAddress: string): Promise<string> => {
  try {
    ...
    const adjustedBalance = Number(balanceStr) / Math.pow(10, decimals)
...
export const getContractInfo = async (web3: any, contractAddress: string): Promise<any> => {
  try {
    ...
    const adjustedTotalSupply = Number(totalSupplyStr) / Math.pow(10, Number(decimals))
...
export const getContractABI = async (contractAddress: string, chainId: number): Promise<any[]> => {
  try {
    ...
    const apiKey = "" // Ideally this should come from user input or environment
...
export const addLiquidity = async (
  ...
  try {
    ...
    const amountAMin = Math.floor(Number(amountAWei) * (1 - slippageTolerance / 100)).toString()
    const amountBMin = Math.floor(Number(amountBWei) * (1 - slippageTolerance / 100)).toString()
    ...
    const router = new web3.eth.Contract(routerABI, routerAddress)
    const tx = await router.methods
      .addLiquidity(tokenA, tokenB, amountAWei, amountBWei, amountAMin, amountBMin, sender, deadline)
      .send({ from: sender })
...
export const removeLiquidity = async (
  ...
  try {
    ...
    const router = new web3.eth.Contract(routerABI, routerAddress)
    const tx = await router.methods
      .removeLiquidity(tokenA, tokenB, liquidityWei, amountAMin, amountBMin, sender, deadline)
      .send({ from: sender })
...
export const explainTransaction = async (web3: any, txHash: string, chainId: string): Promise<any> => {
  try {
