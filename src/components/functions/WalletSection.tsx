...
import { RefreshCw, DollarSign, ArrowDownUp, Settings, Wallet, ArrowDown, Plus } from "lucide-react";
...
const commonTokens = {
  ...
  [educhain.id]: [
    { symbol: 'EDU', name: 'EduChain', address: '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE' },
  ]
};
    ...
    if (web3 && address) {
      try {
        ...
        if (selectedToken === 'native' || selectedToken === 'custom') {
          ...
          const tokenAddress = selectedToken === 'custom' && customTokenAddress 
            ? customTokenAddress 
          ...
          if (selectedToken === 'native') {
            ...
            if (web3.utils.isAddress(customTokenAddress)) {
              const balance = await getTokenBalance(web3, customTokenAddress, address);
              setBalance(balance);
            } else {
          ...
          if (tokenInfo && tokenInfo.address !== '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE') {
            const balance = await getTokenBalance(web3, tokenInfo.address, address);
            setBalance(balance);
          } else {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">
                {balanceLoading ? "Loading..." : `${parseFloat(balance).toFixed(4)} ${getNativeSymbolForBalance()}`}
              </p>
            </div>
